...
const LoopComment = ({ visible, onClose, loopId, user }) => {
  const [newComment, setNewComment] = useState('');
  const [comments, setComments] = useState([]);
  const [loopOwner, setLoopOwner] = useState('');
  ...
  const handleCommentDelete = async (comment) => {
     console.log('Silme İşlemi:', { user: user.username, commentUser: comment.username }); // Kontrol için
   
     if (loopOwner === user.username || comment.username === user.username) {
       try {
         ...
         await updateDoc(loopRef, {
           ...
           commentsCount: loopData.commentsCount - 1,
         });
  ...
  const handleCommentLike = async (comment) => {
    ...
    try {
      ...
      const updatedComments = loopData.commentedBy.map((c) => {
      ...
      await updateDoc(loopRef, { commentedBy: updatedComments });
    ...
    if (selectedComment) {
      ...
      try {
        ...
        await updateDoc(loopRef, {
          ...
          commentsCount: loopDoc.data().commentsCount + 1,
        });
  ...
  useEffect(() => {
     const fetchComments = async () => {
          ...
          if (loopDoc.exists()) {
            ...
            const sortedComments = (data.commentedBy || []).map((comment) => ({
              ...comment,
              username: comment.username || 'Anonim', // Varsayılan bir değer ekle
            })).sort(sortComments);
            setComments(sortedComments);
            setLoopOwner(data.owner || '');
          }
        };
